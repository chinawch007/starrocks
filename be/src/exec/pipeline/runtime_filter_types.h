...
// Used to merge runtime in-filters and bloom-filters generated by multiple HashJoinBuildOperator instances.//哦，文档里说的，多个pipeline的filter合并。
...
// not take effects on operators in front of LocalExchangeSourceOperators before they are merged into a total one.//分区规则是什么，跟joiner的filter有什么区别？
...
public:
    ...
    StatusOr<bool> add_partial_filters(
        ...
        // merge
        if (1 == _num_active_builders--) {//只有最后一个才这么搞
