...
#include "exec/pipeline/hashjoin/merge_join_build_operator.h"
#include "exec/pipeline/hashjoin/merge_join_probe_operator.h"
#include "exec/pipeline/hashjoin/merge_joiner_factory.h"
...
pipeline::OpFactories MergeJoinNode::decompose_to_pipeline(pipeline::PipelineBuilderContext* context) {
    ...
    MergeJoinerParam param(pool, _hash_join_node, _id);
    ...
    //两边用的是同一个joiner
    auto probe_op =
            std::make_shared<HashJoinProbeOperatorFactory>(context->next_operator_id(), id(), merge_joiner_factory);
